...
//-------------
//mrakodrapy
 procedure panorama(var pole: tTabulka;od:integer);
 var i,j,hodnota,pomocna:integer;
   a:array[1..RIADKOV] of integer;
 begin
      pomocna:=0;
 for j:=1 to STLPCOV    do begin
   hodnota:= od -1;
     pomocna:=pomocna+1;
  for i:=1 to RIADKOV
do begin
 ...
 writeln;
                       ...
                       writeln('juh');
  for pomocna:=1 to RIADKOV
do begin  writeln;
  for i:=1 to a[pomocna] do  write('*');
  end;
  writeln; writeln('sever');
  for pomocna:=RIADKOV downto 1 do begin
    writeln;
    for i:=1 to a[pomocna] do write('*');
    end;

   pomocna:=0;
 for i:=1 to RIADKOV    do begin
   hodnota:= od -1;
     pomocna:=pomocna+1;
  for j:=1 to STLPCOV
do begin
...
begin
  ...
  readln(dolna, horna);
  inicializuj(mojepole,dolna,horna);
  zobraz(mojepole);
  panorama(mojepole,dolna);
  readln;
end.
